...
# Subset for 18 donors (MHGA identified as genotyping error by ASE)

mkdir genotype/loseq18

# in R:
library(tidyverse)
meta<- data.table::fread(here::here("data/samplekey_485.txt"), data.table = FALSE)
samps <- meta %>% select(donor_id) %>% distinct()
samps <- samps[samps != "LOSQ-MHGA"]
writeLines(samps, here::here("genotype/loseq18/samples_list_18.txt"))

# terminal
module load bcftools/1.9
bcftools view -Oz --samples-file genotype/loseq18/samples_list_18.txt genotype/vcfs_post_imp_phasing/loseq19.FINAL.vcf.gz -o genotype/loseq18/loseq18.FINAL.vcf.gz

bcftools index -t genotype/loseq18/loseq18.FINAL.vcf.gz
...
PLINK_BINARY='genotype/loseq18/loseq18.FINAL'
...
bcftools query -f '%CHROM\t%POS\t%ID\t%REF\t%ALT\n' genotype/loseq18/loseq18.FINAL.vcf.gz > genotype/loseq18/loseq18.FINAL_lookuptable.txt

gzip genotype/loseq18/loseq18.FINAL_lookuptable.txt
...
cp genotype/loseq18/loseq18.FINAL.nosex genotype/loseq18/loseq18.FINAL.txt

mkdir -p genotype/loseq18/pca/fig

module load plink/1.90-b3.29
PLINK_BINARY='genotype/loseq18/loseq18.FINAL'
INDIV='genotype/loseq18/loseq18.FINAL.txt'
OUT_DIR='genotype/loseq18/pca'
...
# ended up plotting in R, ancestry labeled plot doesn't output correctly --> DO AFTER PROJECT 1KG

module load R/3.6.1

PREFIX='LOSEQ'
HIGHLIGHT_SAMPLES='data/samplekey_508.txt'
EVEC_FILE='genotype/loseq18/pca/smartpca.pca.evec'
EVAL_FILE='genotype/loseq18/pca/smartpca.eval'
FIG_OUT_DIR='genotype/loseq18/pca/fig/'
ANCESTRY_FILE='genotype/loseq18/pca/1kg/loseq.knn_populations_1kg.txt'
...
mkdir genotype/loseq18/overlapping_variants_1kg

# ran sbatch scripts/vcf_processing/vcf_intersection.sh:

SAMPLES=('genotype/loseq18/loseq18.FINAL.vcf.gz' '../data/1kg/phase3_GRCh38/merged/merged_genotypes.1kg_phase3_grch38.vcf.gz')
OUT_DIR=genotype/loseq18/overlapping_variants_1kg/

# compressed using: bcftools view genotype/loseq18/overlapping_variants_1kg/0000.vcf -Oz -o genotype/loseq18/overlapping_variants_1kg/loseq18.overlap.vcf.gz
bcftools view genotype/loseq18/overlapping_variants_1kg/0001.vcf -Oz -o genotype/loseq18/overlapping_variants_1kg/1kg.overlap.vcf.gz
# do same for 1kg, output results to: loseq18.overlap.vcf.gz and 1kg.overlap.vcf.gz
# 5771755 sites overlap



2. Merge loseq and 1kg

mkdir -p genotype/loseq18/pca/1kg

# ran sbatch scripts/vcf_processing/vcf_merge.sh:

SAMPS=('genotype/loseq18/overlapping_variants_1kg/loseq18.overlap.vcf.gz' 'genotype/loseq18/overlapping_variants_1kg/1kg.overlap.vcf.gz')
OUTDIR='genotype/loseq18/pca/1kg'

# output results: genotype/loseq18/pca/1kg/loseq18.1kg.merge.vcf.gz
...
module load plink/1.90-b3.29
VCF='genotype/loseq18/pca/1kg/loseq18.1kg.merge.vcf.gz'
...
FILE='genotype/loseq18/pca/1kg/loseq18.1kg.merge.vcf.gz.filtered.ld_pruned'
...
# FIX INPUTS IN RSTUDIO
sbatch --job-name=plot_pca --mem=10G --mail-type=END,FAIL --mail-user=mmartorella@nygenome.org --output=logs/200914_novaseq/vcfs/stdout/x1a.pca_plot_merged_%j.out --error=logs/200914_novaseq/vcfs/stderr/x1a.pca_plot_merged_%j.error scripts/qtl/pca_plot_projected_onto_1kg.R
...
### genotype covar file: analysis/qtl/covs/loseq18.genotype.covariates.txt
